...
class _RemoteRosContext:
    ...
    def __init__(self) -> None:
        ...
        self.cmd_pub = self.node.create_publisher(Twist, '/cmd_vel_raw', 10)
        self.stop_pub = self.node.create_publisher(Empty, '/picarx/stop_request', 10)
...
class RemotePicarx:
    ...
    def set_motor_speed(self, motor: int, speed: float) -> None:
        ...
        # Approximation because ROS control is speed + steering, not per-wheel torque.
    ...
    def stop(self):
        ...
        self._ctx.stop_pub.publish(Empty())
